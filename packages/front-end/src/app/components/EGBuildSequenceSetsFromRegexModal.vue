<script setup lang="ts">
  ...
  async function confirmCreate(): Promise<void> {
    ...
    try {
      // Create one set per request (same path as "Build sequence set") so a slow or
      ...
      for (const s of activeSets.value) {
        try {
          ...
          console.error(`Failed to create sequence set for ${s.sampleId}`, e);
        }
      }

      if (created === 0) {
        toast.error('Failed to create sequence sets. Check your connection and try again.');
        return;
      }
      if (failedSamples.length) {
        toast.warning(`Created ${created} sequence set(s); ${failedSamples.length} failed.`);
      } else {
        toast.success(`Created ${created} sequence sets`);
      }
      emit('created');
      close();
    } finally {
...
<template>
  ...
  >
    <div class="flex max-h-[85vh] flex-col p-6">
      <h2 class="mb-1 text-lg font-medium">Group files into sequence sets</h2>
      <p class="mb-4 text-sm text-gray-500">
        Apply a filename regex to split {{ fileKeys.length }} selected file(s) into sequence sets.
      </p>
      ...
      <div class="mt-4 flex justify-between gap-2 border-t border-gray-200 pt-4">
        ...
        <UButton v-else :loading="submitting" :disabled="!activeSets.length" @click="confirmCreate">
          Create {{ stats.total }} sequence sets
        </UButton>
      </div>
    </div>
  </UModal>
