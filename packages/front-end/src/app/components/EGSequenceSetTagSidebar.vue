<script setup lang="ts">
  ...
  const standardTagIdToChipCount = computed((): Record<string, number> => {
    ...
    for (const s of setsMatchingSearch.value) {
      for (const tid of standardTagIdsForSet(s.SequenceSetId)) {
  ...
  const untaggedChipCount = computed(
    () => setsMatchingSearch.value.filter((s) => !standardTagIdsForSet(s.SequenceSetId).length).length,
  );
  ...
  const deleteTagSecondaryMessage = computed(() => {
    ...
    if (n === 0) {
      return 'This will permanently delete the tag. It is not currently applied to any sequence sets in this lab.';
    }
    if (n === 1) {
      return 'This will permanently delete the tag and remove it from 1 sequence set in this lab.';
    }
    return `This will permanently delete the tag and remove it from ${n} sequence sets in this lab.`;
  });
  ...
  function tagAssociationCount(tagId: string): number {
    let count = 0;
    for (const s of props.sequenceSets) {
      if ((props.setIdToTagIds[s.SequenceSetId] ?? []).includes(tagId)) count++;
    }
    return count;
  }
...
<template>
  <nav
    ...
    aria-label="Sequence set tag filters"
  >
    <div class="p-2">
      <button
        type="button"
