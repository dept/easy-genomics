import type { SequenceSetGroupingStatus, SequenceSetLayout } from '../types/easy-genomics/sequence-sets';

export type RegexGroupingFileRole = 'read1' | 'read2' | 'reads' | 'reference_fasta' | 'extra';

export type ProposedSequenceSetFile = {
  fileName: string;
  role: RegexGroupingFileRole;
};

export type ProposedSequenceSet = {
  sampleId: string;
  files: ProposedSequenceSetFile[];
  status: SequenceSetGroupingStatus;
...
function inferLayout(files: ProposedSequenceSetFile[]): SequenceSetLayout {
...
function inferStatus(files: ProposedSequenceSetFile[]): SequenceSetGroupingStatus {
...
export function groupFilenamesByRegex(
  fileNames: string[],
  regexPattern: string,
): { sets: ProposedSequenceSet[]; unmatched: string[] } {
  let regex: RegExp;
  try {
  ...
  const bySample = new Map<string, ProposedSequenceSetFile[]>();
  ...
  const sets: ProposedSequenceSet[] = [];
...
export function buildContentsSummary(files: ProposedSequenceSetFile[]): string {
