...
describe('LaboratorySequenceSetService.bulkCreateSequenceSets', () => {
  let svc: LaboratorySequenceSetService;
  let mockCopy: jest.Mock;

  beforeEach(() => {
    jest.clearAllMocks();
    svc = new LaboratorySequenceSetService();
  ...
  it('rejects copy jobs from a bucket that does not match the laboratory', async () => {
    ...
    await expect(
      svc.bulkCreateSequenceSets(lab, 'user-1', 'my-bucket', {
        importLabel: 'import',
        sequenceSets: [],
...
describe('LaboratorySequenceSetService.generateDataCollectionSampleSheet', () => {
  let svc: LaboratorySequenceSetService;
  ...
  let listSequenceSetFilesSpy: jest.SpyInstance;
  ...
  beforeEach(() => {
    ...
    listSequenceSetFilesSpy = jest.spyOn(svc, 'listSequenceSetFiles');
  });

  it('paginates sequence set files when building the sample sheet', async () => {
    const lab = labFixture();
    ...
    mockGetItem.mockImplementation(async (input: { Key?: Record<string, AttributeValue> }) => {
      ...
      if (key.Sk === `DATA_COLLECTION#${collectionId}`) {
        return {
          Item: marshall({
            LaboratoryId: 'lab-1',
            Sk: `DATA_COLLECTION#${collectionId}`,
      ...
      if (key.Sk === `SEQUENCE_SET#${setId}`) {
        return {
          Item: marshall({
            LaboratoryId: 'lab-1',
            Sk: `SEQUENCE_SET#${setId}`,
            SequenceSetId: setId,
            Name: 'SampleA',
            Layout: 'single_end',
          }),
    ...
    mockQueryItems.mockResolvedValue({
      Items: [marshall({ SequenceSetId: setId, Gsi1Pk: 'lab-1#DC#dc-1' })],
    });
...
describe('LaboratorySequenceSetService.addSequenceSetIdToFileRow', () => {
  ...
  it('returns quietly when the set id is already on the file row', async () => {
    mockUpdateItem.mockRejectedValueOnce(new ConditionalCheckFailedException({ message: 'c', $metadata: {} }));
    await (
      svc as unknown as { addSequenceSetIdToFileRow: (...args: unknown[]) => Promise<void> }
    ).addSequenceSetIdToFileRow(
      'lab-1',
...
describe('LaboratorySequenceSetService.adjustDataCollectionSetCount', () => {
  let svc: LaboratorySequenceSetService;
  let mockUpdateItem: jest.Mock;
  ...
  it('uses atomic ADD for sequence set count updates', async () => {
    await (
      svc as unknown as { adjustDataCollectionSetCount: (...args: unknown[]) => Promise<void> }
    ).adjustDataCollectionSetCount('lab-1', 'dc-1', 2, 'user-1');
...
describe('LaboratorySequenceSetService.listUnlinkedBucketObjects', () => {
  let svc: LaboratorySequenceSetService;
  let mockListTransactionInputs: jest.Mock;
  let mockGetSequenceSetIdsForFileRefs: jest.Mock;
  ...
  beforeEach(() => {
    ...
    mockGetSequenceSetIdsForFileRefs = jest.fn().mockResolvedValue(
      new Map([
    ...
    jest.spyOn(svc, 'getSequenceSetIdsForFileRefs').mockImplementation(mockGetSequenceSetIdsForFileRefs);
  });

  it('returns only objects that are not linked to a sequence set', async () => {
