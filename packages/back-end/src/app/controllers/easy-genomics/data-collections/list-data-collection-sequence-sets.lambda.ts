...
export const handler: Handler = async (
  ...
  try {
    ...
    const dataCollectionId = event.queryStringParameters?.dataCollectionId;
    if (!laboratoryId || !dataCollectionId) throw new RequiredIdNotFoundError();

    await assertDataCollectionsAccess(event, laboratoryId);
    const setIds = await sequenceSetService.listDataCollectionSequenceSetIds(laboratoryId, dataCollectionId);
    return buildResponse(200, JSON.stringify({ SequenceSetIds: setIds }), event);
  } catch (err: any) {
    console.error(err);
